 ...
 * the License.
 ******************************************************************************/
package vip.zhouxin.ureport.core.definition.dataset;
...
public class SqlDatasetDefinition implements DatasetDefinition {
	public static final String TYPE = "sqlData";
	...
	public Dataset buildDataset(Map<String,Object> parameterMap, Connection conn){
		...
		if(sqlExpression!=null){
			sqlForUse= SQLUtils.executeSqlExpr(sqlExpression, context);
		}else{
			sqlForUse = sqlHandler(context,sqlForUse);
		}
	...
	public static String sqlHandler(Context context, String sqlForUse) {
		...
		while (matcher.find()) {
			...
			String result = SQLUtils.executeSqlExpr(expr, context);
