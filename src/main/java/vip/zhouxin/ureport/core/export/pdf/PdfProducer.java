 ...
 * the License.
 ******************************************************************************/
package vip.zhouxin.ureport.core.export.pdf;
...
import vip.zhouxin.ureport.core.export.Producer;
import vip.zhouxin.ureport.core.model.Image;
import vip.zhouxin.ureport.core.model.*;
...
public class PdfProducer implements Producer {
	...
	private com.itextpdf.text.Image buildPdfImage(String base64Data, int width,int height) throws Exception{
		...
		try{
			...
			if(width==0){
				width=Double.valueOf(imgWidth).intValue();
			}
			if(height==0){
				height=Double.valueOf(imgHeight).intValue();
			}
