...
export function tableToJson(fileName,context){
    ...
    for(let i=0;i<countRows;i++) {
        for (let j = 0; j < countCols; j++) {
            let c = context.getCell(i,j);
            c.name = context.getCellName(i,j);
            let cellJson = jQuery.extend({}, c);
            cellJson.chartWidget = null;
            cellJson.crossTabWidget = null;
            cells.push(cellJson);
        }
    }
    let dataSourcesJson = []
    for (let i = 0; i < context.reportDef.datasources.length; i++) {
        dataSourcesJson.push({
            name: context.reportDef.datasources[i].name,
            type: context.reportDef.datasources[i].type,
            datasets: context.reportDef.datasources[i].datasets,
            driver: context.reportDef.datasources[i].driver,
            url: context.reportDef.datasources[i].url,
            username: context.reportDef.datasources[i].username,
            password: context.reportDef.datasources[i].password,
            beanId: context.reportDef.datasources[i].beanId
        })
    }
    return {
        reportFullName:fileName,
        paper:context.reportDef.paper,
        header:context.reportDef.header,
        footer:context.reportDef.footer,
        searchForm:context.reportDef.searchForm,
        cells:cells,
        rows:rows,
        columns: cols,
        datasources: dataSourcesJson,
        searchFormXml:context.reportDef.searchFormXml,
    }
}
