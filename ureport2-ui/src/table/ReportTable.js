...
export default class ReportTable{
    constructor(container,callback){
        ...
        let file=utils.getParameter('template');
        if(!file || file===''){
            file='classpath:template/template.ureport.json';
        }else{
            window._reportFile=decodeURIComponent(file);
        }
    ...
    loadFile(file,callback){
        ...
        $.ajax({
            ...
            data:JSON.stringify({
                template: decodeURIComponent(file)
            }),
            success:function(reportDef){
                ...
                _this.hot.render();
                if(file!=='classpath:template/template.ureport.json'){
