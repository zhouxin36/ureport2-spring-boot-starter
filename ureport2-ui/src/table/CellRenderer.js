...
export function afterRenderer(td,row,col,prop,value,cellProperties){
    ...
    if(cellDef.expand === 'Down'){
        let iconUrl = '../../icons/expr-expand-down.svg'
        if(valueType==='dataset'){
            iconUrl='../../icons/expand-down.svg';
        }
        $td.prepend(`<image src="${iconUrl}"></image>`);
    }else if(cellDef.expand === 'Right'){
        let iconUrl='../../icons/expr-expand-right.svg';
        if(valueType==='dataset'){
            iconUrl='../../icons/expand-right.svg';
        }
        $td.prepend(`<image src="${iconUrl}" style="display: block;"></image>`);
    }else{
        if(valueType==='dataset'){
            let iconUrl='../../icons/property.svg';
            $td.prepend(`<image src="${iconUrl}" style="display: inline-block;"></image>`);
        }else if(valueType==='expression'){
            let iconUrl='../../icons/expression.svg';
            $td.prepend(`<image src="${iconUrl}" style="display: inline-block;"></image>`);
        }
    }
    if(cellStyle.align){
